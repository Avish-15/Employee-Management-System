...
const EmployeeList = () => {
  ...
  const deleteEmployee=(e,id)=>{
    ...
    .then((res)=>{
        if(employees){
          setEmployees((prevEelemnet)=>{
            return prevEelemnet.filter((employee)=>employee.id !==id);
          });
        }
    });
  };
    ...
    <div className='container mx-auto my-8'>
      ...
      <div className='h-12'>
        ...
        className='rounded font-semibold  text-white bg-slate-600 border-b px-6 py-2 hover:bg-slate-800' >
            Add Employee
            </button>
      </div>
